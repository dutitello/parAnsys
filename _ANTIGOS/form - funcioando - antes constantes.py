	...
	def Run(self, maxIter, tol=10**-3, dh=0.05, diff='center', meth='HLRF', avoidExcess=False):
		...
		* 99: undefined error!
		...
		if self._ANSYS:
			# Set Length
			if diff is 'center':
				# Lenght is 2*NAnsysVars + 1
				Ns = 1 + len(self.ansys.varInNames)*2

			elif diff is 'forward':
				# Lenght is NAnsysVars + 1
				Ns = 1 + len(self.ansys.varInNames)
		...
		# correlMat start as an eye matrix
		NInVars = len(self.variabledistrib)
		# NOutVars are ANSYS out vars
		NOutVars = 0
		self.correlMat = np.eye(NInVars)
		...
		for cycle in range(1, 1+self.controls['maxIter']):
			...
			#
			matStd = np.zeros([NInVars, NInVars])
			vecMean = np.zeros(NInVars)
			vecPts = np.zeros(NInVars)
			for eachVar in self.variabledistrib:
				# Variable id
				id = varId[eachVar]
				# Current X point
			...
			curVecRedPts = matLinv.dot(vecPts-vecMean)
			...
			while True is True:
				if diff is 'center':
					# size is 1+2*NInVars
					matEvalPts = np.zeros([(1+2*NInVars), (NInVars+NOutVars)])
					# First line is just X
					matEvalPts[0, 0:NInVars] = vecPts
					# Run all lines from [1,end] with step=2
						# curId is current varId
					curId = 0
					for eachLine in range(1, 1+2*NInVars, 2):
						# vecdh is not zero on current var item
						vecdh = np.zeros(NInVars)
						vecdh[curId] = dh
						# Odd line is +h
						matEvalPts[eachLine, 0:NInVars] = vecMean + matL.dot(curVecRedPts + vecdh)
						# and now -h
						matEvalPts[eachLine+1, 0:NInVars] = vecMean + matL.dot(curVecRedPts - vecdh)
						curId += 1

				elif diff is 'forward':
					# size is 1+NInVars
					matEvalPts = np.zeros([(1+NInVars), (NInVars+NOutVars)])
					# First line is just X
					matEvalPts[0, 0:NInVars] = vecPts
					curId = 0
					for eachLine in range(1, 1+NInVars):
						# vecdh is not zero on current var item
						vecdh = np.zeros(NInVars)
						vecdh[curId] = dh
						matEvalPts[eachLine, 0:NInVars] = vecMean + matL.dot(curVecRedPts + vecdh)
			...
			if meth is 'HLRF' or meth is 'iHLRF':
				...
				if meth is 'iHLRF':
					...
					#	gradYy = grad(y.y^T)
					gradYy = np.zeros(NInVars)

					# 	gradMy = grad(m(y))
					gradMy = np.zeros(NInVars)
					...
					for step in range(ceil(maxnk/stepnk)):
						...
						# points
						matEvalPts = np.zeros([curlen, (NInVars+NOutVars)])
						lambdks = np.zeros(curlen)

						for eachnk in range(curlen):
							lambdks[eachnk] = par_b**nk
							matEvalPts[eachnk, 0:NInVars] = vecMean + matL.dot(curVecRedPts + lambdks[eachnk]*dk)
							nk += 1
